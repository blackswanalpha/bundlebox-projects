...
# A worklist service and the UI in front of it

One collection, six states, and every refusal says which field it refused — the boring parts done properly are the product.

**Falsified by:** one response shape a client cannot predict from the contract, or one refusal that does not name its field
**Unit of work:** one surface, with its contract, its refusals and a corpus that proves it against a running process

## Surfaces

- **health** — is it up at all, and does it know how much it holds
- **items** — the thing the product is about
- **web** — the only surface a person actually sees

## Lanes

- **L01** the store: durability behind one map — `src/store.js`, projected 34.1k
- **L02** items: what an item is, and the only place that decides — `src/items.js`, projected 34.7k
- **L03** the wire: routes in one table, handlers that return rather than write — `src/server.js`, projected 36.2k
- **L04** the corpus: what the running system does, not what the files say — `test/server.test.js`, projected 36.6k
