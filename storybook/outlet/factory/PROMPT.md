...
- **scan** — what is wrong, with evidence, for nothing
- **compile** — what to do about it, packed to one window
- **route** — who does it, where, in what wave
- **run** — the only verb that spends
- **measure** — what a session used and was spared, separately

## Lanes

- **L01** the core: paths, config, fs, exec, store, util, args, log — `src/core`, projected 50.5k
- **L02** the detectors and the shared corpus they all read once — `src/detectors`, projected 99.4k
- **L03** the local actuators that close what they can before a model is asked — `src/actuators`, projected 54.6k
- **L04** compile: findings to units, packed to one window with quoted regions — `src/compile`, projected 74.4k
- **L05** route: units to lanes and waves, conflicts as affinity — `src/route`, projected 53.3k
- **L06** run: lanes on the chosen agent, env allowlist, acceptance as the verdict — `src/run`, projected 52.1k
- **L07** tokens: estimate, calibrate, probe, price, and the session bill — `src/tokens`, projected 72.8k
- **L08** designlabs: the doctrine, the declared system and the gate that holds it — `src/designlabs`, projected 82.3k
- **L09** wire: the instruction block, the MCP entry and the hooks, per agent — `src/wire`, projected 54.2k
- **L10** the CLI: one entrypoint, every verb, a broken module is a row not a crash — `src/cli.js`, projected 41.0k
